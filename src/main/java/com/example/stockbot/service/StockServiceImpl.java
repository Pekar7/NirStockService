...
public class StockServiceImpl implements StockService {
    ...
    public Candles getCandles(String ticker) {
        ...
        var context = api.getMarketContext()
                .getMarketCandles(figi, OffsetDateTime.now().minusMonths(8), OffsetDateTime.now(), CandleResolution.DAY);
        ...
        for (int i = 0; i < listContext.size(); i++) {
            data.add(new String[]{String.valueOf(listContext.get(i).getC()), "Candle", String.valueOf(listContext.get(i).getTime().toZonedDateTime())});
        }
        ...
        getCandlesBySMA(ticker, listContext);
        getCandlesByEMA(ticker, listContext);
        return context.join().get();
    }


    @Override
    public Resource loadAsResource(String ticker) {

        Candles a = getCandles(ticker);
        try {
            Path file = Paths.get("src/main/resources/data/AnalyseDataSet.csv");
    ...
    @Override
    public File getCandlesBySMA(String ticker, List<Candle> listContext) {
        String figi = getStockByTicker(ticker).getFigi();
        ...
        SMAIndicator shortSma = new SMAIndicator(closePrice, 100); //период сглаживания
...
//    }
    ...
    @Override
    public File getCandlesByEMA(String ticker, List<Candle> listContext) {
        String figi = getStockByTicker(ticker).getFigi();
